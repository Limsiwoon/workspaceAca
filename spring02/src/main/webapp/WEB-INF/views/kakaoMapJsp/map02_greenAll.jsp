...
for(let i=0; i<total; i++) {
	...
	geocoder.addressSearch(address, function(result, status) {
		if (status === kakao.maps.services.Status.OK) {
			...
			//let content = addrs[counter].content;
			/* 인포윈도우 : BigBox 참고
			let content = '<div class ="label"><span class="left"></span><span class="center">'
...
let yLat = new Array(); // y 좌표를 담을 배열
let xLng = new Array(); // x 좌표를 담을 배열
...
//	  console.log("xLat=>"+xLng);
	for(let i=0; i<total; i++) {
		...
		let coords = new kakao.maps.LatLng(yLat[i], xLng[i]);
