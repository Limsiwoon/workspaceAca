...
function idOk(){
	//1) 
	//opener.document.getEelementById('id').value=document.getEelementById('id').value;
	opener.document.getElementById('id').value="${param.id}";
	// => EL 활용: jsp 문서에서는 script 구문의 문자열 내부에 있는 EL은 처리해줌
	//${param.id} 사용 하면 ``제이쿼리를 사용한다는 의미 
	//2)
...
<body>
   ...
   <c:if test="${idUse=='T'}">
   	...
   	<br><button onclick="idOk()">ID 선택</button>&nbsp;&nbsp;
   </c:if>
    <c:if test="${idUse=='F'}">
   	${param.id} 는/은 사용이 불가능 합니다 &nbsp;&nbsp;
   	<br>다시 입력하세요~ 
      ...
      -->
   	<script>
   		...
   		//_self = window 로 열기. _blanck = opener 로 열기 
